...
function countVowels(str) {
  ...
  return [...str].filter((char) => letters.includes(char)).length;
}
...
function getCardId(value) {
  ...
  const weight = value.slice(0, -1); // Витягуємо значення рангу
  const suit = value.slice(-1); // Витягуємо масть

  return weights.indexOf(weight) + suits.indexOf(suit) * weights.length;
}
